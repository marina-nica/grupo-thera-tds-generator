...
const COLORS = {
  primary: '#1C355E', // escuro institucional
  accent: '#3cb4e5', // azul ciano
  light: '#ABE1FA',
  neutral: '#2271ac',
  bgZebra: '#f1f1f1'
};
...
export default function TheramartLayout({ data }: { data: TDSData }) {
  const theme = BRAND_THEMES.theramart;
    ...
    <Page size="A4" style={[styles.page, { fontFamily: theme.fontFamily }]}>
      ...
      <View style={styles.section}>
         ...
         <View style={styles.table}>
            ...
            <View style={styles.row}>
               <Text style={styles.colHeader}>Quantidade por caixa / Quantity per box / Cantidad por caja</Text>
               <Text style={styles.colData}>{data.logistics.unitsPerBox}</Text>
            </View>
         </View>
      </View>

      <Footer brand="theramart" borderColor={COLORS.accent} />
    </Page>
  );
}
